...
local AccountId = 2;
local auth_id = crypt.hash(tostring(game:GetService("Players").LocalPlayer.UserId), "sha256")
...
function Verify()
	...
	local status, result = pcall(function() 
            return game:HttpGetAsync("https://api-gateway.platoboost.com/v1/public/whitelist/2/" .. auth_id .. "?s");
        end);
            ...
            elseif string.find(result, "false") then
                if (#key > 0) then
	                local redeemResponse = request({
	                    Url = "https://api-gateway.platoboost.com/v1/authenticators/redeem/2/" .. auth_id .. "/" .. key,
	                    Method = "POST"
	                });
...
script.Parent.Home.Popup.Add.MouseButton1Click:Connect(function()
    ...
    writefile("d_ios_script_dir/" .. script.Parent.Home.Popup.Title.TextBox.Text, script.Parent.Home.Popup.Source.TextBox.Text)
